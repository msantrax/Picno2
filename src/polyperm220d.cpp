...
long getRelativeTick() {
    return (bspcounter - inittick) * 16;
}

double getSeconds() {
    return ((bspcounter - inittick)*16) / 1000;
}

bool isAlarmTime(uint8_t index) {
    return bspcounter > alarms[index];
}
...
uint8_t getKeyCode() {
    ...
    if (!locateEvent(KEYCODE_SIG)) return 0;
    //og(INFO, "Keycode event =%d @getKeycode\r\n", mainevents.front().simple);
...
int main() {
    ...
    pc.printf("=            Versao Compilada em  %s %s  -- serie 4  \r\n", __DATE__, __TIME__);
    pc.printf("===============================================================================\r\n\n");
    ...
    subscribe(PROPERTIES, SETUSER_SIG);
    ...
    while (true) {
        ...
        if (state == RESET) {
            ...
            publish(event_t(DRAWMESSAGE_SIG, 0, new drawmessage_evt(76, 0, 10, false, false, true, 0, new std::string("          23"))));
            drawShapeRequest(SHP_STATUSBAR, 0U, 48U, false);
            //state=BANNER;
            ...
            if (locateEvent(INPUTANSWER_SIG)) {
                ...
                if (ia->val == 1992.0) {
                    ...
                    properties->setAllowLevel(2);
                    state = BANNER;
                } else if (ia->val == 4556.0) {
                    analise->setUser(1.0);
                    properties->setAllowLevel(1);
                    state = BANNER;
                } else if (ia->val == 6969.0) {
                    state = FODASE;
                    analise->setUser(-1.0);
                } else {
                    analise->setUser(0.0);
                    properties->setAllowLevel(0);
                    state = BANNER;
                }
                //log(FINE, "User logged with %f - user=%f\r\n", ia->val, analise->getUser());
        ...
        else if (state == FODASE) {
           
            analise->initVars();  
            simpleEventRequest(INITANALISE_SIG, 0);
          
            publish(event_t(DRAWDIALOG_SIG, 0, new dialog_evt(
                    0,
                    255,
                    BANNER,
                    BANNER,
                    new std::string("SISTEMA\nFOI\nRESTABELECIDO")
                    )));
            state = WAITFODASE;
        } else if (state == WAITFODASE) {
            if (locateEvent(DIALOGANSWER_SIG)) {
                temp = mainevents.front().simple;
                if (temp == 255) {
                    //log(FINE, "User restablished system\r\n");
                    analise->setUser(-1);
                    properties->saveProfile(); 
                }
                state = BANNER;
            }
        }
            ...
            if (analise->getUser() == 1 || analise->getUser() == 2) {
                //log(FINE, "Init Canvas em adm\r\n");
            ...
            publish(event_t(DRAWMESSAGE_SIG, 0, new drawmessage_evt(76, 0, 10, false, false, true, 0, new std::string("          26"))));
