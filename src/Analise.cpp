...
Analise::Analise() {
	...
	apmap.push_back(pmaps(Properties::BLAINEDEV  ,Properties::P_BOOLEAN    ,(char*)"autostore"                      ,&autostore         ,(char*)"true"));
	...
	apmap.push_back(pmaps(Properties::BLAINEDEV  ,Properties::P_BOOLEAN    ,(char*)"autoprint"                      ,&autoprint         ,(char*)"true"));    
	...
	apmap.push_back(pmaps(Properties::BLAINEDEV  ,Properties::P_FLOAT      ,(char*)"vcell"							,&vcell				,(char*)"154.7144"));
	...
	// Calibration Services ======================================================================================================
		...
		if(choiceavailable){
			...
			if (choiceanswer == 0){
				...
				if (spherevol < 0){
					log(INFO, "Using Defined sphere Volume = %f", spherevol);
				}
				else{
					spherevol = atof(sval.c_str());
				}
		...
		if (!flip){
			if (lruns.size() <= cycles){			
				if (verifyCyclesRSD(rsdbypressure, runsaveraged)){
					...
					if (cycles == 0){
						publish(event_t(DRAWMESSAGE_SIG, 0, new drawmessage_evt(04,8,120,true,false,true,1,new std::string("Executando analise simples"))));
					}
					else{
						publish(event_t(DRAWMESSAGE_SIG, 0, new drawmessage_evt(04,8,120,true,false,true,1,getCycleString(lruns.size(), cycles))));
					}
...
std::string * Analise::getFinalString(uint8_t mode, uint8_t slot, bool media){
	...
	if (media){
		if (mode == 0){
			...
			sprintf(buf, "RATIO: %6.4f / %6.4f", temp1, temp2);
		}
	}
	else{
		if (mode == 0){
...
std::string * Analise::getPressureString(bool added){
	char buf[32];
	if (added){
		sprintf(buf, "Expandir para:%7.4f PSI", lastpressure);
	}
	else{
		sprintf(buf, "Carr. com:%7.4f PSI", lastpressure);
	}
