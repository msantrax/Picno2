...
namespace blaine {
    ...
    glcd::glcd() {
        ...
        canvas_widget_map[0][0] = 3;
        canvas_widget_map[0][1] = 41;
        canvas_widget_map[0][2] = 0;
        canvas_widget_map[1][0] = 42;
        canvas_widget_map[1][1] = 43;
        canvas_widget_map[1][2] = 0;
        canvas_widget_map[2][0] = 85;
        canvas_widget_map[2][1] = 43;
        canvas_widget_map[2][2] = 0;
        canvas_widget_map[3][0] = 3;
        canvas_widget_map[3][1] = 41;
        canvas_widget_map[3][2] = 3;
        canvas_widget_map[4][0] = 43;
        canvas_widget_map[4][1] = 43;
        canvas_widget_map[4][2] = 3;
        canvas_widget_map[5][0] = 85;
        canvas_widget_map[5][1] = 43;
        canvas_widget_map[5][2] = 3;
    ...
    void glcd::setNumeric(bool numeric) {
        kbdnumeric = numeric;
    }
            ...
            case SERVICE_INPUT:
                if (!kbdevents.empty()) {
                    if (inptdesc->error == 1) {
                        ...
                        //log(FINE, "service status returned : %d\n\r", service_status);
                        kbdevents.pop();
                        if (service_status == 1) {
                            clearInput();
                            ...
                            //log(FINE, "Servicing Dialog Input normal comand\r\n");


                        } else if (service_status == 5) {
                            //log(FINE, "Servicing Dialog Input escape comand\r\n");
                            inputanswer_evt *ans = new inputanswer_evt(-1, &inptdesc->data[0]);
                            event_t evt(INPUTANSWER_SIG, service_status, ans);
                            publish(evt);
                            publish_kbdcode = true;
                            service_state = SERVICE_NONE;//
    ...
    double glcd::getFloatInput() {
        return inptdesc->result;
    }
    ...
    uint8_t glcd::navigateInput(uint8_t keycode) {
        ...
        //log(FINE, "Navigate input was called with code %d\r\n", keycode);
        ...
        if (inptdesc->functionbar) {
            ...
            switch (keycode) {
                case 0x0d:
                    ...
                    break;
                case 0x11:
                    // UP Key -> interpret as escape command
                    //log(FINE, "Input dialog up key pressed\r\n");
                    return 5;
                    break;
